...
import { removeTerminal } from "../../TerminalFeature/Actions/RemoveTerminal";
...
const TerminalsEventsHandler = lifecycle({
  ...
  componentDidMount() {
    this.onTerminalsChange = this.onTerminalsChange.bind(this);
    TerminalsController.eventEmitter.on("change", this.onTerminalsChange);
  },
  componentWillUnmount() {
    TerminalsController.eventEmitter.removeListener(
      "change",
      this.onTerminalsChange,
    );
  },
});
  ...
  connect(
    ...
    (dispatch: (action: Object) => void, props) => ({
      onStartPlan: (plan: PlanConfig) => {
        dispatch(runTask(plan, { type: "integrated" }));
      },
      ...
      onPackageTerminalRemove: (id: string) => {
