  ...
  return from(plugins).pipe(
    map(plugin =>
      defer(() => {
        ...
        return Observable.fromPromise(isPackage).map(test => {
...
export function findPackages(
  ...
  if (!rootPath.match(/(.)*\.(.)*/gi)) {
    ...
    const getEntries = Observable.bindNodeCallback(
    ...
    return entriesObs
      .map(entries => {
        return Observable.defer(() => {
          if (entries == null) return Observable.empty();
          else
